FUNCTION clumpy2, G, plot

 COMMON galaxy_block
 rimg = galaxy_image

 r_e = pet_rad_cir2(0.2, G)

 width = (1.5*r_e)/ 6.0  ; smoothing length

 if (width gt G.psf/G.scale) then begin
  ; smooth image
     srimg = FILTER_IMAGE(rimg, smooth=width, /ALL_PIXELS)
     c1 = rimg - srimg
 ...
 ; set center region to zero
     ...
     dist_circle, cir, galaxy_npix, G.axc, G.ayc ; region to measure S
